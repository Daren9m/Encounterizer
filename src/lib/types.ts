...
export interface EncounterMonster {
  monster: Monster;
  count: number;
  /** Recipe role retained after slot filling so table notes can name the
   *  intended job even when the same stat block fills more than one slot. */
  recipeRole?: string;
}

export type EncounterBeatTrigger =
  | { kind: 'round'; round: number }
  | { kind: 'leader-hp'; percent: number }
  | { kind: 'enemies-remaining'; count: number }
  | { kind: 'ally-at-zero' }
  | { kind: 'manual'; label: string };

export interface EncounterRecipeBeat {
  id: string;
  title: string;
...
export interface EncounterRecipeObjective {
  title: string;
  summary: string;
  success: string;
  failure: string;
...
export interface EncounterRecipePlan {
  version: 1;
  recipeId: string;
  recipeName: string;
  objective: EncounterRecipeObjective;
  setup: string[];
  beats: EncounterRecipeBeat[];
  forecast: {
    headline: string;
    guidance: string[];
    caveat: string;
...
export interface Encounter {
  ...
  tactics?: string;
  treasure?: string;
  /** Structured, table-ready instructions generated by an encounter recipe. */
