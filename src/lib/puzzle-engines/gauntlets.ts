...
export const gauntlets: PuzzleFamily = {
  ...
  generate({ levers, rng }: EngineInput): EngineOutput {
    ...
    return {
      ...
      dmBrief,
      readAloud: `${cap(pick(pack.sensory, rng))}. You have found ${first.name} — ${withArticle(pick(pack.materials, rng))} chamber where ${first.hazard}. The only way out: ${first.escape}.`,
      handout: {
        kind: 'text',
        title: 'Gauntlet Notes',
        body: hazards.map((h, i) => `${phaseCount > 1 ? `Phase ${i + 1}: ` : ''}${h.name} — ${h.hazard}\nEscape: ${h.escape} (${h.skills.join('/')})`).join('\n\n'),
      },
